...
| 深度专注 | 主页开关启动：全屏沉浸（隐藏系统栏）+ 隐藏放弃按钮（返回键弹强提醒）+ 默认自动播放白噪音 + 完成金币按时长翻倍，另有专属成就「心流初体验 / 深潜大师」 |
| 屏幕固定 | 深度专注自动请求 Screen Pinning（startLockTask）：Home / 多任务 / 返回全部失效，长按返回才可退出，系统确认框期间不误判离开 |
| 桌面接管 | 侧边栏实验开关：启用清单 HOME 别名（默认禁用）并在系统「主屏幕应用」里选择 Focus 后，按 Home 键回到 Focus 而非原桌面，可随时关闭恢复 |
...
- [x] 白噪音 / 深度专注模式
- [x] 屏幕固定 + 桌面接管 + 浮窗遮挡检测 + 碎裂记录（v0.8.0）
